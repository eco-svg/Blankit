...
══════════════════════════════ */
(function () {
  const pvModal     = document.getElementById('preVerifyModal');
  const pvSchool    = document.getElementById('pvSchool');
  ...
  const pvSubmitBtn = document.getElementById('pvSubmitBtn');
  const pvCancelBtn = document.getElementById('pvCancelBtn');
  const avcVerifyBtn = document.getElementById('avcVerifyBtn');

  if (!pvModal) return;

  function openPvModal() {
    if (pvSchool)   pvSchool.value   = '';
    if (pvGrade)    pvGrade.value    = '';
    if (pvLocation) pvLocation.value = '';
    pvStatusEl.className = 'pv-status hidden';
    pvStatusEl.textContent = '';
    pvSubmitBtn.disabled    = false;
    pvSubmitBtn.textContent = 'Submit →';
    pvModal.classList.remove('hidden');
    setTimeout(() => pvSchool && pvSchool.focus(), 60);
  }

  function closePvModal() { pvModal.classList.add('hidden'); }

  function setPvStatus(msg, ok) {
    pvStatusEl.textContent = msg;
    pvStatusEl.className   = 'pv-status ' + (ok ? 'pv-ok' : 'pv-err');
  }

  avcVerifyBtn && avcVerifyBtn.addEventListener('click', openPvModal);
  pvCancelBtn  && pvCancelBtn.addEventListener('click', closePvModal);
  pvModal.addEventListener('click', function (e) { if (e.target === pvModal) closePvModal(); });

  pvSubmitBtn && pvSubmitBtn.addEventListener('click', async function () {
    const school   = pvSchool   ? pvSchool.value.trim()   : '';
    const grade    = pvGrade    ? pvGrade.value.trim()    : '';
    const location = pvLocation ? pvLocation.value.trim() : '';
    if (!school || !grade) { setPvStatus('School and grade are required.', false); return; }
    pvSubmitBtn.disabled    = true;
    pvSubmitBtn.textContent = 'Submitting…';
    try {
      const res  = await fetch('/auth/pre-verify-student', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (res.ok) {
        setPvStatus('✓ Saved — we\'ll review after you create your account.', true);
        if (avcVerifyBtn) {
          avcVerifyBtn.textContent = '✓ Submitted';
          avcVerifyBtn.disabled    = true;
        }
        setTimeout(closePvModal, 2200);
      } else {
        setPvStatus(data.error || 'Submission failed. Try again.', false);
        pvSubmitBtn.disabled    = false;
        pvSubmitBtn.textContent = 'Submit →';
      }
    } catch {
      setPvStatus('Network error. Try again.', false);
      pvSubmitBtn.disabled    = false;
      pvSubmitBtn.textContent = 'Submit →';
    }
  });
